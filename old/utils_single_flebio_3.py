...
def create_df(data, country_mapping, sitc_hierarchy):
    partner_data = []
    node_attributes = {}
    ...
    for country, country_data in data.items():
        ...
        export_commodities_sitc = list(export_commodities_sitc)
        ...
        node_attributes[country_mapped] = {
            ...
            'Export Commodities SITC': export_commodities_sitc,
...
def plot_top_level_commodities(sorted_counter, sitc_hierarchy):
    # Create a mapping from level 2 categories to top-level categories
    level_2_to_top_level = {}
    
    # Build the mapping based on the hierarchy
    for top_level, subcategories in sitc_hierarchy.items():
        for level_2 in subcategories:
            level_2_to_top_level[level_2] = top_level

    # Aggregate counts at the top-level category
    top_level_counter = Counter()
    for level_2, count in sorted_counter.items():
        top_level = level_2_to_top_level.get(level_2)
        if top_level:
            top_level_counter[top_level] += count

    # Sort top-level categories by count
    sorted_top_level = sorted(top_level_counter.items(), key=lambda item: item[1], reverse=True)
    
    # Unzip the top-level categories and their counts
    categories, counts = zip(*sorted_top_level)
    ...
    fig.update_layout(
        title="Top-Level Export Commodities Occurrences",
        xaxis_title="Top-Level Category",
        yaxis_title="Occurrences",
        xaxis=dict(tickfont=dict(size=4)),  # Adjusted font size for better readability
        height=500
    )
    ...
    # Return the sorted counter of top-level categories
    return dict(sorted_top_level)
